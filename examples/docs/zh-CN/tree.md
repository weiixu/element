...
:::demo
```html
<gj-tree :data="data" :props="defaultProps" @node-click="handleNodeClick"></gj-tree>
      ...
      renderContent(h, { node, data, store }) {
        return (
          <span class="custom-tree-node">
            <span>{node.label}</span>
            <span>
              <gj-button size="mini" type="text" on-click={ () => this.append(data) }>Append</gj-button>
              <gj-button size="mini" type="text" on-click={ () => this.remove(node, data) }>Delete</gj-button>
            </span>
          </span>);
      }
    }
